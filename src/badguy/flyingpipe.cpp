...
FlyingPipe::FlyingPipe(const ReaderMapping& reader) :
  BadGuy(reader, "images/creatures/pipebadguys/flying_pipe/flying_pipe.sprite"),
  total_time_elapsed(),
  puff_timer()
{
...
bool
FlyingPipe::collision_squished(GameObject& object)
{
...
FlyingPipe::active_update(float dt_sec)
{
  ...
  if (puff_timer.check()) {
    ...
    Sector::get().add<SpriteParticle>("images/particles/invisible.sprite",
                                           "default",
